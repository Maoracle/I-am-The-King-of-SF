...
pygame.display.set_caption("我是影魔王")
...
get_king_sound = pygame.mixer.Sound("sound/get_bullet.wav")
get_king_sound.set_volume(0.2)
...
#添加敌人
def add_axe(group1, group2, num):
    for i in range(num):
        e1 = enemy.Axe(bg_size)
        group1.add(e1)
        group2.add(e1)
    
def add_tide(group1, group2, num):
    for i in range(num):
        e2 = enemy.Tide(bg_size)
        group1.add(e2)
        group2.add(e2)
        
def add_roshan(group1, group2, num):
    for i in range(num):
        e3 = enemy.Roshan(bg_size)
        group1.add(e3)
        group2.add(e3)
